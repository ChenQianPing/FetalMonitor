...
//孕妇登录
-(void)didCheckPwLoginWithCmd:(NSString *) cmd machineNo:(NSString*)machineNo userName:(NSString*)userName completionCallBack:(callBack) callBack {
    NSString *params = [NSString stringWithFormat:@"cmd=%@&machineNo=%@&userName=%@",  cmd, machineNo,userName];
    
    [self getByApiName:@"" andParams:params andCallBack:^(id obj) {
...
-(void)getByApiName:(NSString *)apiName andParams:(NSString *)params andCallBack:(callBack)callback {
    ...
    NSString *apiPath = [NSString stringWithFormat:@"http://www.zgzqjh.com/ajax.aspx?action=App"];
 //   NSString *accessToken = [[NSUserDefaults standardUserDefaults] objectForKey:@"accessToken"];
    //   NSString *accessToken = @"null";
    if (params) {
      //  NSString *normalParams = [NSString stringWithFormat:@"access_token=%@",accessToken];
        apiPath = [apiPath stringByAppendingFormat:@"&%@",params];
    ...
    NSURLSessionTask *task = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        NSError *err;
        id receiveData = [NSJSONSerialization JSONObjectWithData:aData options:0 error:&err];
...
-(UIImage *) getImageFromURL:(NSString *)fileURL {
    ...
    NSString *apiPath = [NSString stringWithFormat:@"http://www.zgzqjh.com%@", fileURL];
    NSLog(@"执行图片下载函数%@", apiPath);
    
    UIImage * result;
    NSData * data = [NSData dataWithContentsOfURL:[NSURL URLWithString:apiPath]];
...
-(void)postByApiName:(NSString *)apiName andParams:(NSString *)params andCallBack:(callBack)callback {
    ...
    NSString *apiPath = [NSString stringWithFormat:@"http://www.zgzqjh.com/ajax.aspx%@", apiName];
   // NSString *accessToken = [[NSUserDefaults standardUserDefaults] objectForKey:@"accessToken"];
